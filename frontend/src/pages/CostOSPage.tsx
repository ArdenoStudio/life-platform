    ...
    <div className="space-y-5">
      ...
      <PulsePanel>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-paper/80">{t(locale, 'costOfLifeWeights')}</p>
          </div>
          {setActivePage ? (
            <button
            ...
            <p className="text-sm font-semibold text-paper/70">{t(locale, 'seeTrustForMethodology')}</p>
          )}
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {mvpWeights.map(({ key, percent, kicker }) => {
            ...
            return (
              <div key={key} className="rounded-lg border border-white/12 bg-white/8 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-paper/65">{t(locale, kicker)}</p>
                    <p className="mt-1 text-3xl font-semibold text-paper">{percent}%</p>
                  </div>
                </div>
              </div>
            )
      ...
      <section className="grid gap-5 xl:grid-cols-[0.78fr_1.22fr]">
        <PulsePanel tone="muted">
          ...
          <h2 className="mt-3 font-display text-4xl font-semibold leading-tight text-paper">
            {formatLkrLocale(costCommand?.total_monthly_lkr, localeTag(locale))}
          </h2>
          <p className="mt-3 text-sm leading-6 text-paper/75">{t(locale, 'publicBudgetEstimate')}</p>
        ...
        <PulsePanel>
          ...
          <h2 className="mt-1 text-2xl font-semibold text-paper">{t(locale, 'basketComponents')}</h2>
        ...
        <PulsePanel>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {costCommand?.items.map((item) => (
              <div key={item.key} className="rounded-lg border border-white/12 bg-white/8 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                  {item.source_keys.slice(0, 4).map((key) => (
                    <span key={`${item.key}-${key}`} className="rounded-md border border-white/12 bg-white/10 px-2 py-1 text-[11px] font-semibold text-paper/70">
                      {key}
                    </span>
                  ))}
                </div>
                <div className="mt-4 flex items-end justify-between gap-3">
                  <span className="text-2xl font-semibold text-paper">{formatLkrLocale(item.monthly_lkr, localeTag(locale))}</span>
        ...
        <div className="grid gap-5">
          <PulsePanel>
            <div className="flex items-center gap-2">
              <PlugZap className="h-5 w-5 text-gold" aria-hidden="true" />
              <h2 className="text-xl font-semibold text-paper">{t(locale, 'utilitiesAndLpg')}</h2>
            </div>
            <div className="mt-4 space-y-3">
              {[...(utilities?.electricity ?? []), ...(utilities?.water ?? []), ...(utilities?.gas ?? [])].slice(0, 5).map((item) => (
                <div key={item.key} className="flex items-start justify-between gap-4 rounded-lg border border-white/12 bg-white/8 p-3">
                  <span>
                    <span className="block font-semibold text-paper">{item.label}</span>
                    <span className="block text-xs text-paper/70">{item.note}</span>
                    <span className="mt-1 inline-flex rounded-md border border-white/12 bg-white/10 px-2 py-1 text-[11px] font-semibold text-paper/70">
                      {item.source_key}
                    </span>
                  </span>
                  <span className="text-right font-semibold text-paper">{formatLkrLocale(item.amount_lkr, localeTag(locale))}</span>
                </div>
              ))}
            </div>
          </PulsePanel>
